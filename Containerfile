...
RUN if [[ "${FEDORA_MAJOR_VERSION}" == "rawhide" ]]; then \
    ...
    # rpm-ostree install \
    #    patch rsync tar unzip wget which diffutils python3 perl-base perl-Data-Dumper perl-File-Compare perl-File-Copy perl-FindBin perl-IPC-Cmd perl-JSON-PP perl-Thread-Queue \
    #    perl-Time-Piece re2c libreoffice nodejs npm minicom elvish nu virt-viewer virt-manager virt-install nodejs npm minicom && \
